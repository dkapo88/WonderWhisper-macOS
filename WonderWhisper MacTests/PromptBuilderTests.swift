...
struct ScreenContextTermExtractorTests {
    ...
    @Test func extractorKeepsNamesProductsAcronymsAndTicketIds() {
        let text = """
        Slack thread with Dane Kapoor and Luis about WonderWhisper Mac.
        Please compare OpenRouter Voice, Soniox V4, Apple Intelligence, and CORE-759.
        The WWMac-lite context path should preserve API names and GPT-4o-mini-transcribe.
        """
        ...
        #expect(terms.contains("WonderWhisper Mac"), "Terms: \(terms)")
        ...
        #expect(terms.contains("WWMac-lite"), "Terms: \(terms)")
    ...
    @Test func preprocessorFallsBackToLocalTermsWhenAppleIntelligenceIsDisabled() async {
        ...
        let result = await preprocessor.preprocess(
            ocrText: "Dane Kapoor is testing OpenRouter Voice with Soniox V4 in WWMac-lite."
        )
        ...
        #expect(result?.contextText.contains("WWMac-lite") == true)
    }
